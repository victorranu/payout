...
const App = () => {
  ...
  const [totals, setTotals] = useState({ wolf: 0, skins: 0 });
  ...
  useEffect(() => {
    ...
    setTotals({ wolf: totalWolfPoints, skins: totalSkins });
    ...
    const calculatedResults = players.map(p => {
      ...
      const totalMoney = totalNetPoints * unitPrice;
    ...
    calculateTransactions(calculatedResults);

  }, [players, unitPrice]);

  // Settlement Algorithm: Match Winners and Losers
  const calculateTransactions = (data) => {
    // Deep copy to avoid mutating state during calculation
    let debtors = data
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border-l-4 border-green-600">
          ...
          <p className="text-slate-500 text-sm mt-1">
            Enter Wolf points and Skins won. Logic assumes "Poker style" buy-in for Wolf and "Pays Everyone" style for Skins.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="mt-4 pt-4 border-t border-slate-100 text-xs text-slate-400">
               <p>Math Check: Total Net should be 0. Actual: {results.reduce((a,b)=>a+b.totalNetPoints,0).toFixed(2)}</p>
            </div>
          </div>
